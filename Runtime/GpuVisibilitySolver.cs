...
{
    /// <summary>
    /// GPU-based visibility detection using CommandBuffer rendering (runtime compatible).
    /// Renders triangle IDs as vertex colors from multiple viewpoints,
    /// reads back pixels to determine which triangles are visible.
    ///
    /// IMPORTANT: Add "Hidden/Internal-Colored" to Project Settings > Always Included Shaders,
    /// or assign a custom vertex-color shader via the material parameter.
    /// </summary>
    public static class GpuVisibilitySolver
    {
            ...
            Action<float, string> onProgress = null)
        {
            ...
            // Float RT avoids sRGB precision loss
        ...
        private static Material CreateIDMaterial()
        {
            ...
            if (shader == null)
            {
                Debug.LogWarning("[GpuVisibility] Hidden/Internal-Colored not found. "
                    + "Add it to Always Included Shaders. Using fallback.");
